...
The project rules from `CLAUDE.md` are incorporated here. Scoped rules also live in
`scripts/AGENTS.md`, `src/sync/AGENTS.md`, and `src/xlsx/AGENTS.md`; read the relevant
one before changing those areas (the sync rules also apply to `electron/sync-server.js`).
Keep shared project guidance consistent with the corresponding `CLAUDE.md` files;
agent-specific configuration paths and environment variables must describe the actual
configuration rather than being renamed mechanically.
...
taking each other down live in `scripts/AGENTS.md`**, next to the scripts themselves.

`scripts/push-reminder.sh` is configured in Claude Code’s `Stop` hook
(`.claude/settings.json`) and only prints a line when source files are newer than
`.push-stamp`. The existing `.codex/hooks.json` is a copy that still references
`CLAUDE_PROJECT_DIR`; its presence alone does not verify a working Codex hook.
It must stay a
...
quantities as numbers rather than free text) — see `src/xlsx/AGENTS.md`.
...
`electron/sync-server.js`), and so does its guidance: **`src/sync/AGENTS.md`** covers the
